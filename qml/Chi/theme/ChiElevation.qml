...
QtObject {
    ...
    function _params(level) {
        return _shadowParams["level" + level] || _shadowParams.level0;
    }

    function verticalOffset(level) {
        return _params(level)[0];
    }
    function blurRadius(level) {
        return _params(level)[1];
    }
    function shadowOpacity(level) {
        return _params(level)[2];
    }
