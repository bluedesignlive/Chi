...
Item {
    ...
    Rectangle {
        ...
        opacity: root._modal && root._state > 0 && root._state < 3 ? Math.min(sheet.height / Math.max(root.minHeight, 1) * 0.32, 0.32) : 0
            ...
            Item {
                ...
                DragHandler {
                    ...
                    onCentroidChanged: {
                        ...
                        if (dt > 0)
                            root._vel = (centroid.position.y - root._prevY) / dt * 1000;
                        root._prevY = centroid.position.y;
                        root._prevT = now;
                    }
                }
    ...
    function _snapAfterDrag() {
        ...
        if (best <= 0 && dismissible) {
            close();
        } else if (best >= maxHeight && expandable) {
            _state = 2;
            _animTo(maxHeight);
            expanded();
        } else {
    ...
    Keys.onEscapePressed: {
        if (_state > 0 && _state < 3 && dismissible)
            close();
    }
