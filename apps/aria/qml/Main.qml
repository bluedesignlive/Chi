...
ChiApplicationWindow {
    ...
    FileDialog {
        ...
        mode: "openMultiple"
        title: "Open Audio"
        nameFilters: ["Audio files (*.mp3 *.wav *.flac *.ogg *.m4a *.aac *.opus *.wma *.aiff *.ape)", "All files (*)"]
    ...
    Row {
            ...
            ListView {
                ...
                delegate: Item {
                    ...
                    Rectangle {
                        ...
                        radius: 12
                        color: del.isCurrent ? colors.primaryContainer : itemMouse.containsMouse ? colors.surfaceContainerHighest : "transparent"
                        ...
                        Row {
                            ...
                            Column {
                                ...
                                Text {
                                    ...
                                    text: del.isCurrent ? (audio.playing ? "Playing" : "Paused") : del.isQueued ? "Up Next" : ""
                                    font.family: ChiTheme.fontFamily
                                    font.pixelSize: 11
        ...
        Rectangle {
            ...
            CompactPlayer {
                anchors.fill: parent
                visible: hasFile && compactMode
                isPlaying: audio.playing
                progress: audio.duration > 0 ? audio.position / audio.duration : 0
                currentMs: audio.position
                totalMs: audio.duration
                currentVolume: audio.volume
                isMuted: audio.muted
                fileName: folderPlayer.currentFileName
                currentIndex: folderPlayer.currentIndex
                totalCount: folderPlayer.count
                hasNext: folderPlayer.hasNext
                hasPrevious: folderPlayer.hasPrevious
                onPlayPauseTriggered: audio.togglePlayPause()
                onPreviousTriggered: folderPlayer.previous()
                onNextTriggered: folderPlayer.next()
                onSeekTriggered: function (pos) {
                    audio.seek(pos);
                }
                onVolumeAdjusted: function (val) {
                    audio.volume = val;
                }
                onMuteToggled: function (val) {
                    audio.muted = val;
                }
            }
                ...
                Column {
                    ...
                    Row {
                        ...
                        IconButton {
                            ...
                            variant: "tonal"
                            size: "medium"
                            enabled: folderPlayer.hasPrevious
                            onClicked: folderPlayer.previous()
                        }
                        IconButtonToggle {
                            anchors.verticalCenter: parent.verticalCenter
                            icon: "play_arrow"
                        ...
                        IconButton {
                            ...
                            variant: "tonal"
                            size: "medium"
                            enabled: folderPlayer.hasNext
                            onClicked: folderPlayer.next()
                        }
                    }
                }
            ...
            DropArea {
                ...
                onDropped: function (drop) {
                    if (drop.hasUrls)
                        folderPlayer.loadUrls(drop.urls);
                }
            }
        }
    }
    ...
    Shortcut {
        sequence: "Space"
        onActivated: if (hasFile)
            audio.togglePlayPause()
    }
    Shortcut {
        sequence: "Left"
        onActivated: if (hasFile)
            audio.seekBackward(5000)
    }
    Shortcut {
        sequence: "Right"
        onActivated: if (hasFile)
            audio.seekForward(5000)
    }
    Shortcut {
        sequence: "Ctrl+Left"
        onActivated: if (folderPlayer.hasPrevious)
            folderPlayer.previous()
    }
    Shortcut {
        sequence: "Ctrl+Right"
        onActivated: if (folderPlayer.hasNext)
            folderPlayer.next()
    }
    Shortcut {
        sequence: "Up"
        onActivated: audio.volume = Math.min(1, audio.volume + 0.05)
    }
    Shortcut {
        sequence: "Down"
        onActivated: audio.volume = Math.max(0, audio.volume - 0.05)
    }
    Shortcut {
        sequence: "M"
    ...
    Shortcut {
        sequence: "Ctrl+M"
        onActivated: compactMode = !compactMode
    }
    Shortcut {
        sequence: "Ctrl+B"
